...
module.exports = function({
  ...
  app.post('/query', async (req, res) => {
    let queryResponse = await axios
      .get(
        `http://localhost:${tendermintPort}/abci_query?data=""&opts={"trusted":true}`
      )
      .catch(e => {
        console.log(e)
      })
    ...
    // completely trusting the response for now.
    // in lotion v0.1.7 and earlier, we at least verified the headers
    // for now, we're completely trusting the full node
    res.json(value)
  })
  return app
}
