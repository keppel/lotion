...
// Timeouts can probably be adjusted to longer times to collect more transactions from mempool
      config_lines =  `
[consensus]
...
// peer_gossip_sleep_duration = 1000
     ...
     require('fs').readFile(join(lotionPath, 'config.toml'), (err, content) => {
        if (err) throw err;
        if (content.indexOf("[consensus]")>-1) {
          console.log("Config already modified.")
        } else {
          require('fs').appendFile(join(lotionPath, 'config.toml'), config_lines, function (error) {
            if (err) throw err;
          });
        }
      })
