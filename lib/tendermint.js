...
module.exports = function({
  ...
  return new Promise(async (resolveNodeRunning, rejectNodeRunning) => {
    ...
    if (genesis) {
      fs.writeFileSync(join(lotionPath, cfgPath, 'genesis.json'), stringify(genesis))
    }
    if (keys) {
      let validatorJsonPath = join(lotionPath, cfgPath, 'priv_validator.json')
      ...
      fs.writeFileSync(validatorJsonPath, JSON.stringify(newValidatorJson), { encoding: 'utf8' })
    }

    // add app hash to genesis
    let newGenesis = Object.assign(
      {},
      JSON.parse(fs.readFileSync(join(lotionPath, cfgPath, 'genesis.json'))),
      {
        app_hash: initialAppHash
      }
    )
    fs.writeFileSync(join(lotionPath, cfgPath, 'genesis.json'), stringify(newGenesis))
    ...
    // let tendermintProcess = {}; // TODO: Add ability to separate tendermint process with flag
    let shuttingDown = false
    console.log('./tendermint ' + tpArgs.join(' '))
