...
module.exports = function configureABCIServer({
  ...
  abciApp.info = async function(req) {
    ...
    if (lastState.appHash == rootHash.toString('hex')) {
      ...
      try {
        ...
        lastState = JSON.parse(await storeDb.get(lastBlockHeight))
        console.log(`State found at ${lastBlockHeight-1}`)
      } catch(err) {
        console.log(`No state found at ${lastBlockHeight-1}`)
        lastBlockHeight = 0
        lastState = { store }
      }
      rootHash = await getRoot(lastState.store)
      console.log(rootHash.toString('hex'))
      console.log(lastState.appHash)
    }
