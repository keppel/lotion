...
module.exports = function configureABCIServer({
  ...
  abciApp.checkTx = async function(req, cb) {
    let rawTx = req.checkTx.tx
    try {
      let tx = JSON.parse(Buffer.from(rawTx, 'base64').toString());
      // let tx = JSON.parse(decodedTx) //decodeTx(rawTx) // left4dead - remove unnecessary functions
  ...
  abciApp.deliverTx = async function(req, cb) {
    let rawTx = req.deliverTx.tx
    try {
      let tx = JSON.parse(Buffer.from(rawTx, 'base64').toString());
      // let tx = decodeTx(rawTx)  // left4dead - remove unnecessary functions
  ...
  abciApp.query = async function(req, cb) {
    try {
      let selectedStore = ""
      if (req.query.path) {
        let result = jp.query(store, req.query.path)
        selectedStore = JSON.stringify(result)
      } else {
        selectedStore = JSON.stringify(store)
      }

      let queryResponse = {
        value: new Uint8Array(new Buffer(selectedStore)),
        height: chainInfo.height - 1,
        proof: '', // TODO: utilize proof
        key: '',
        index: 0,
        code: 0,
        log: '',
  ...
  abciApp.info = async function(req, cb) {
    ...
    cb({ lastBlockAppHash: new Uint8Array(rootHash), lastBlockHeight: startingBlockHeight })
  }
