...
module.exports = function configureABCIServer({
  ...
  abciApp.query = function(req, cb) {
    ...
    let resValue = Buffer.from(stringify(stateToHash))
    // all these fields are required by `tendermint lite` currently:
    let queryResponse = {
      value: resValue,
      height: { low: chainInfo.height - 1, high: 0 },
      proof: Buffer.from('12', 'hex'),
      key: Buffer.from('12', 'hex'),
      index: { low: 0, high: 0 },
      code: 0,
      log: ''
      // todo: proofs.
