...
function runTx(stateMachine, appState, tx, chainInfo, allowMutation = false) {
  ...
  stateMachine(hookedState, tx, chainInfo)
  return stateMutated
}
...
module.exports = function configureABCIServer(
  ...
  abciApp.checkTx = function(req, cb) {
    ...
    let isValid = runTx(stateMachine, appState, tx, chainInfo, false)
  ...
  abciApp.deliverTx = function(req, cb) {
    ...
    let tx = decodeTx(rawTx)
    let isValid = runTx(stateMachine, appState, tx, chainInfo, true)
    if (isValid) {
